...
void show_obj_modifiers(struct obj_data *obj, struct char_data *ch)
{
  ...
  if (OBJ_FLAGGED(obj, ITEM_BLESS) && AFF_FLAGGED(ch, AFF_DETECT_ALIGN))
    send_to_char(ch, " ..brilla azul!");

  if (OBJ_FLAGGED(obj, ITEM_MAGIC) && AFF_FLAGGED(ch, AFF_DETECT_MAGIC))
    send_to_char(ch, " ..brilla amarillo!");

  if (OBJ_FLAGGED(obj, ITEM_GLOW))
    send_to_char(ch, " ..Posee un aura suave brillante!");

  if (OBJ_FLAGGED(obj, ITEM_HUM))
    send_to_char(ch, " ..Emite un sonido zumbante!");
}
...
void diag_char_to_char(struct char_data *i, struct char_data *ch)
{
  struct {
    ...
    { 100, "Se encuentra en excelente condición."			},
    {  90, "tiene unos rasguñitos."			},
    {  75, "tiene unos cuantos moretones y algunas heridas."		},
    {  50, "tiene notables heridas."			},
    {  30, "tiene horrendas y grandes heridas y rasguñotes!."	},
    {  15, "luce bastante doloroso."				},
    {   0, "su condición es deplorable."			},
    {  -1, "está sangrando profusamente de sus enormes heridas."	},
  };
...
void look_at_char(struct char_data *i, struct char_data *ch)
{
  ...
  if (!ch->desc)
   ...
   if (i->player.description)
    ...
    act("No ves algo especial en $m.", FALSE, i, 0, ch, TO_VICT);
  ...
  if (ch != i && (IS_THIEF(ch) || GET_LEVEL(ch) >= LVL_IMMORT)) {
    ...
    if (!found)
      send_to_char(ch, "No puedes ver nada.\r\n");
  }
}
...
void list_one_char(struct char_data *i, struct char_data *ch)
{
  const char *positions[] = {
    " yace aquí, muerto.",
    " yace aquí, mortalmente herido.",
    " yace aquí, incapacitado.",
    " yace aquí, desorientado.",
    " está durmiendo aquí.",
  ...
  if (AFF_FLAGGED(i, AFF_HIDE))
    send_to_char(ch, " (oculto)");
  if (!IS_NPC(i) && !i->desc)
    send_to_char(ch, " (linkless)");
  if (!IS_NPC(i) && PLR_FLAGGED(i, PLR_WRITING))
    send_to_char(ch, " (escribiendo)");

  if (GET_POS(i) != POS_FIGHTING)
    send_to_char(ch, "%s", positions[(int) GET_POS(i)]);
  else {
    if (FIGHTING(i)) {
      send_to_char(ch, " está aquí, peleando ");
      if (FIGHTING(i) == ch)
	send_to_char(ch, "TÚ!");
      else {
	if (IN_ROOM(i) == IN_ROOM(FIGHTING(i)))
	  ...
	  send_to_char(ch,  "Alguien se acaba de ir.");
      }
...
void do_auto_exits(struct char_data *ch)
{
  ...
  send_to_char(ch, "%s[ Salidas: ", CCCYN(ch, C_NRM));
