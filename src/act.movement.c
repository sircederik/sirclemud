...
int do_simple_move(struct char_data *ch, int dir, int need_specials_check)
{
  ...
  if (!AFF_FLAGGED(ch, AFF_SNEAK))
    act("$n ha llegado.", TRUE, ch, 0, 0, TO_ROOM);
...
int perform_move(struct char_data *ch, int dir, int need_specials_check)
{
  ...
  if (ch == NULL || dir < 0 || dir >= NUM_OF_DIRS || FIGHTING(ch))
    ...
    send_to_char(ch, "No puedes ir por ahí...\r\n");
  else if (EXIT_FLAGGED(EXIT(ch, dir), EX_CLOSED)) {
    if (EXIT(ch, dir)->keyword)
      send_to_char(ch, "La %s parece estar cerrado.\r\n", fname(EXIT(ch, dir)->keyword));
    else
      send_to_char(ch, "Parece estar cerrado.\r\n");
  } else {
    if (!ch->followers)
...
int find_door(struct char_data *ch, const char *type, char *dir, const char *cmdname)
{
  ...
  if (*dir) {			/* a direction was specified */
    if ((door = search_block(dir, dirs, FALSE)) == -1) {	/* Partial Match */
      send_to_char(ch, "no es una dirección.\r\n");
      return (-1);
    }
	...
	if (isname(type, EXIT(ch, door)->keyword))
	  ...
	  send_to_char(ch, "No veo %s ahí.\r\n", type);
	  return (-1);
        }
      } else
	return (door);
    } else {
      send_to_char(ch, "De verdad no veo como puedas %s algo ahí.\r\n", cmdname);
      return (-1);
    }
...
ACMD(do_enter)
{
  ...
  if (*buf) {			/* an argument was supplied, search for door
    ...
    send_to_char(ch, "No hay %s aquí.\r\n", buf);
  } else if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_INDOORS))
    send_to_char(ch, "Ya estás en el interior.\r\n");
  else {
    ...
    send_to_char(ch, "No puedes encotrara nada para entrar.\r\n");
  }
}
